...
import soundON from "../components/images/sound-on.png"
import soundOFF from "../components/images/sound-off.png"
import coconut from "../components/images/coconut.PNG"
import me from '../components/images/me.PNG'
import hoverSound from "../components/sounds/woosh.wav"
import clickSound from "../components/sounds/click.mp3"
import transitionSound from "../components/sounds/transition.mp3"
import bgMusic from "../components/sounds/background.mp3"
                ...
                <>
                    <mesh>
                        <Html wrapperClass key={key} scale={0.7} rotation={[Math.PI / 2, 0, 0]} position={[1.88, 0, (key + -2) * -0.5]} transform>
                            <div className="group relative cursor-pointer flex items-center justify-center h-[22px] 
                            text-menuText font-black w-[97px] text-center text-sm hover:text-white"
                                onClick={() => menuClicked(item)}
                                onMouseEnter = {()=> isMusic && hoverSoundEffect(item)}
                            >
        ...
        <>
            ...
            <div className='absolute w-full min-h-full bg-gradient top-0 z-20 flex justify-center'>
                    <div className={`p-10 mt-8 md:mt-0 md:max-w-[80%] md:p-20 transition-opacity duration-1000 opacity-0 ${ isTimed && 'delay-300 opacity-100'}`}>
                        ...
                        <div className='fixed group top-[10px] right-[10px] md:top-[50px] md:right-[50px] w-[50px] h-
                        [50px] bg-primary cursor-pointer flex justify-center text-menuText hover: text-white'
                        onClick={() => closePage()} onMouseEnter={() => isMusic && hoverSoundEffect()}>

                            <div className="ablosute h-full bg-menu w-0 right-0 block tansform group-hover:animate-cover"></div>
            ...
            <img id="musicTrigger" alt="sound icon" src={soundOFF} className="absolute opacity-80 w-[40px] h-[40px] z-10
            bottom-10 right-[calc(50vw-20px)] md:bottom-[40px] cursor-pointer" onClick={(e) => backgroundMusic(e)}/>
